...
class TestParseChartData:
    ...
    def test_parse_instrument_data_txt_error(self, caplog):
        sv.parse_instrument_data(test_resources / "example_fail_filetype.txt")
...
class TestPlotXYChart:
    ...
    def test_plot_xy_chart_multiple_y_pass(self, chart_type, data, config):
        config.x_names = []
        config.y_names = ["Col1", "Col2", "Col3"]
        config.chart_type = chart_type
        assert isinstance(sv.plot_xy_chart(data, config), go.Figure)

    @pytest.mark.parametrize("chart_type", ["overlay", "subplots"])
    def test_plot_xy_chart_multiple_x_pass(self, chart_type, data, config):
        config.x_names = ["Col1", "Col2", "Col3"]
        config.y_names = []
        config.chart_type = chart_type
        assert isinstance(sv.plot_xy_chart(data, config), go.Figure)

    @pytest.mark.parametrize("chart_type", ["overlay", "subplots"])
    def test_plot_xy_chart_warn(self, chart_type, config):
