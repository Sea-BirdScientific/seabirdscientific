...
class TestAlignCtd:
    ...
    def test_align_ctd_add_simple_pass_2(self):
        ...
        # Fix last valid sample
        expected_data.measurements['cond0S/m'].values[len(expected_data.measurements['cond0S/m'].values) - 2] = -9.99e-29
...
class TestCellThermalMass:
    ...
    def test_cell_thermal_mass_pass(self):
        ...
        assert(np.allclose(
            expected_data.measurements['cond0S/m'].values, corrected_conductivity, atol=0.000001
        ))
...
class TestWindowFilter:
    ...
    def test_median_filter_exclude_flags(self):
        ...
        expected_pressure = [
            7.405, 7.722, 7.98, 8.197, 8.286, 8.452, 8.654, 8.779, 8.879, 8.987,
            9.169, 9.328, 9.415, 9.605, 9.818, 10.047, 10.305, 10.598, 10.917, 11.082,
            11.418, 11.926, 12.265, 12.615, 12.936, 13.401, 13.681, 13.816, 14.051, 14.275,
            14.481, 14.663, 14.839, 15.017, 15.192, 15.37, 15.556, 15.751, 15.852, 16.062,
            16.408, 16.644, 16.903, 17.18, 17.608, 17.912, 18.068, 18.395, 18.735, 19.08,
            19.418
        ]
        filtered_pressure = dp.window_filter(
            self.pressure,
