...
def contour_from_t_s_p(
    temperature_C, salinity_PSU, pressure_dbar, min_salinity=0, lat=0, lon=0
) -> ContourData:
    ...
    temperature_min = conservative_temperature.min() - (
        0.1 * conservative_temperature.min()
    )
    temperature_max = conservative_temperature.max() + (
        0.1 * conservative_temperature.max()
    )
    ...
    for j in range(0, y_cells):
        potential_density_matrix[j, :] = gsw.rho(
            salinity_vector, temperature_vector[j], 0
        )
...
def contour_from_t_c_p(
    temperature_C, conductivity_mScm, pressure_dbar, min_salinity=0, lat=0, lon=0
) -> ContourData:
    ...
    return contour_from_t_s_p(
        temperature_C, salinity_PSU, pressure_dbar, min_salinity, lat, lon
    )
